...
def cart_update(request, cart_item_id):
    ...
    if request.method == 'POST':
        ...
        if kart.product.stock >= new_quantity:
            ...
            messages.warning(request,"There is no enough in stock, reduce quantity")
    return redirect('cart_item')
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        return render(
            request,
            'store/checkout.html',
            {'cart_items': cart_items, 'total_price': total_price, 'delivery_fee':delivery_fee, 'paystack_fee':paystack_fee},
        )
...
def store_required(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        if hasattr(request.user, 'store'):
            ...
            return HttpResponseForbidden("You must have a store to access this page")
...
def add_product(request):
    if request.method == 'POST':
        if request.POST['name'] and request.POST['price'] and request.FILES['image'] and request.POST['category'] and request.POST['desc'] and request.POST['stock']:
            try:
